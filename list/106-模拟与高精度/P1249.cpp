...
/**
 * 贪心 + 高精度，高精度模板一贴就好（我写的类还挺好用！）
 *
 * 这里主要是贪心，我们如何让分解的数的乘积最大呢？
 * 我们可以发现 n*(n + 1) > (n - 1)*(n + 2)
 * 我们还可以发现 n! < n!*(n + 1)
 * 也就是说我们希望相乘的数字最多，这样答案会更大
 * 那么我们就贪心的希望可以变成2*3*4*5*...*i的形式
 * 那么这样肯定会存在一个i使得这些数字加起来会大于n
 * 那么这时候去掉n - i就好了
 */
#include <iostream>
#include <cstdio>
